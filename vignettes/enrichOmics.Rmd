...
author: |
 ...
 | <small>School of Public Health, City University of New York, New York</small>
date: "July 27, 2017"
abstract: "Overview of existing methods for enrichment analysis of gene 
...
enrichment analysis for data derived from multiple high-throughput omics assays."
email:
output:
...
```
- Required packages: [EnrichmentBrowser][], [ALL][], [hgu95av2.db][], [airway][],
[regioneR][], [BSgenome.Hsapiens.UCSC.hg19.masked][], [MultiAssayExperiment][], 
[readr][], [mogsa][], [limma][], [BiocStyle][], [knitr][], [rmarkdown][]
...
This corresponds to a one-sided Fisher's exact test, yielding here a significant enrichment.
...
resulting in activiation or inhibition of specific target genes.
...
[Goeman and Buehlmann, 2007](https://doi.org/10.1093/bioinformatics/btm051)
classified existing enrichment methods into *competetive* and *self-contained* 
based on the underlying null hypothesis.

- *Competetive* null hypothesis: the genes in the set of interest are at most as
...
question of interest, the vast majority of enrichment methods is competetive.
...
slightly different approach by classifiying methods along the timeline of 
...
cautios interpretation of results is required to derive valid conclusions. 
...
```
Note: we are switching here from `SummarizedExperiment` to `ExpressionSet` as 
functionality in the EnrichmentBrowser is explicitely designed for gene-level 
expression data, for which `ExpressionSet` can be consistently used.
...
`r Biocpkg("limma")` package for differential expression analysis.
...
```{r browseEA, eval=FALSE}
ea.browse(ora.all)
```
<!-- TODO: complex heatmap group colors
-->
...
<!--
TODO:
 Also note the author of SAFE (William Barry) works at DFCI Biostat & Comp Bio and may be there! Would be a good chance to show off your super impressive feature:
```{r}
sbea.methods()
```
-->
...
[Geistlinger et al., 2016](https://doi.org/10.1186/s12859-016-0884-1),
[Alhamdoosh et al., 2017](https://doi.org/10.1093/bioinformatics/btw623). 
See the EnrichmentBrowser
[vignette](http://www.bioconductor.org/packages/release/bioc/vignettes/EnrichmentBrowser/inst/doc/EnrichmentBrowser.pdf),
Section 9: *Combining results*.

<!--
TODO: Why not show it off here? People will be impressed to be able to easily perform and compare results from many different approaches. And, would "comparing" results be a better way to put it than "combining" results? 
-->
...
See the MultiAssayExperiment
[workshop](https://github.com/waldronlab/MultiAssayExperimentWorkshop)
for an introduction. 
...
The dataset contains data for RNA-seq, reverse phase protein array, and
[gistic](https://doi.org/10.1073/pnas.0710052104)
copy number variation.
Sample data is available on BRCA mutation and expression-based subtype
classification.
...
We first make sure that rows (genes/proteins) and columns (patients) are 
matching between the three assays:
...
Despite the availabibilty of multi-omics data resources, bioinformatic and
statistical methods for the analysis of such experiments are still nascent.
This also holds for multi-omics enrichment analysis, where ongoing developments
are anticipated to produce novel methods for this purpose.
